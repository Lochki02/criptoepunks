...
import criptopunk from "./assets/images/criptoepunks.png"
...
function App() {
  ...
  const mint = async() => {
    ...
    const freeMinted = await contract.freeMinted(blockchain.account)
    const paidMinted = await contract.paidMinted(blockchain.account)
    const cost = await contract.cost()

    let freeMints = 2 - Number(freeMinted.toString())
    let paidMints = 0
    let costToPay = "0"

    if(amount > freeMints){
      paidMints = amount - freeMints
      if(paidMints > 2 - Number(paidMinted.toString())) return
      else costToPay = String(Number(cost.toString()) * (paidMints))
    }

    if(freeMints + paidMints <= 4) contract.mint(freeMints, paidMints, { gasLimit: 250000, value: costToPay })
    //else if(Number(paidMinted.toString()) + amount <= 2) contract.mintFree(amount, { gasLimit: 250000, value: String(Number(cost.toString()) * amount) })
  }
  ...
  const increment = async() =>{
    ...
    if(amount + 1 <= 4 - (Number(freeMinted.toString()) + Number(paidMinted.toString()))) setAmount(amount + 1)
  }
      ...
      <div className="main">
        ...
        <div className="side1">
          ...
          <div className="aboutwriting2">
            <p1 className="writing2 blinking">2 FREE, 2 PAID (0.003), 2 PER WALLET<br />10,000 Supply</p1>
          </div>

          {
            ...
            <div className='mintingContainer'>
              <div id='amountContainer'>
                <p className='amountButton' onClick={decrement}>-</p>
                <p id='amount'>{amount}</p>
                <p className='amountButton' onClick={increment}>+</p>
              </div>
...
const ConnectWalletButton = () => {
  ...
  return (
    <button
      className={`connect-wallet${blockchain.account ? ' connected' : ''}`}
      onClick={() => {
    ...
    >
      {blockchain.account
        ? `Connected: ${shortenAddress(blockchain.account)}`
        : 'Connect Wallet'}
    </button>
  );
};
